...
$artifactRoot = Join-Path $repoRoot 'artifacts\v0.22.0-preview'
...
& dotnet publish $modernProject --configuration Release --runtime win-x64 --self-contained true --no-restore -p:PublishSingleFile=true -p:DebugType=None -p:DebugSymbols=false --output $setupPublish
if ($LASTEXITCODE -ne 0)
{
...
$setupSource = Join-Path $installerOutput 'KParser-Sanctum-Setup-Preview-22.exe'
$setupAsset = Join-Path $artifactRoot 'KParser-Sanctum-Setup-Preview-22.exe'
...
try
{
    ...
    & dotnet publish $modernProject --configuration Release --runtime win-x64 --self-contained true --no-restore -p:PublishSingleFile=true -p:SanctumPortable=true -p:DebugType=None -p:DebugSymbols=false --output $portablePublish
    if ($LASTEXITCODE -ne 0)
    {
...
$portableAsset = Join-Path $artifactRoot 'KParser-Sanctum-Portable-Preview-22.zip'
Compress-Archive -Path (Join-Path $portablePackage '*') -DestinationPath $portableAsset -CompressionLevel Optimal
...
$checksumLines = @($setupAsset, $portableAsset) | ForEach-Object {
