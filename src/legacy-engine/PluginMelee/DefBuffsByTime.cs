...
{
    public class DefBuffsByTimePlugin : BasePluginControl
    {
            ...
            ref StringBuilder sb, List<StringMods> strModList)
        {
            ...
            var attackSet = from c in dataSet.Combatants
                            ...
                            let targetActions = c.GetInteractionsRowsByTargetCombatantRelation()
                            select new AttackGroup
                            {
                                Name = c.CombatantName,
                                DisplayName = c.CombatantNameOrJobName,
                                Melee = from n in targetActions
                                        ...
                                        select n,
                                Range = from n in targetActions
                                        where ((ActionType)n.ActionType == ActionType.Ranged &&
                                               ((HarmType)n.HarmType == HarmType.Damage ||
                                                (HarmType)n.HarmType == HarmType.Drain)) &&
                                               mobFilter.CheckFilterMobTarget(n) == true
                                        select n,
                                Spell = from n in targetActions
                                        where ((ActionType)n.ActionType == ActionType.Spell &&
                                               ...
                                               mobFilter.CheckFilterMobTarget(n) == true
                                        select n,
                                Ability = from n in targetActions
            ...
            foreach (var playerInterval in intervalSets)
            {
                if (playerInterval.TimeIntervalSets.Count > 0)
                {
                            ...
                            CollectTimeIntervals.AccuracyBuffNames.Contains(s.SetName))))
                    {
                        ...
                        {
                            if (CollectTimeIntervals.AccuracyDefBuffNames.Contains(intervalSet.SetName))
                            {
                                ...
                                if (intervalSet.TimeIntervals.Count > 0)
                                {
                                    ...
                                    if (inCount > 0)
                                    {
                                        if (!playerHeader)
                                        {
                                            ...
                                            {
                                                Start = sb.Length,
                                                Length = lsDefenseHeader.Length,
                                                Bold = true,
                                                Underline = true
                                            });
                                            sb.Append(lsDefenseHeader + "\n");
                                        }
        ...
        protected override void LoadResources()
        {
            ...
            lsDefense = Resources.Combat.BuffsByTimePluginAttack;
            lsDefenseHeader = Resources.Combat.BuffsByTimePluginAttackHeader;
