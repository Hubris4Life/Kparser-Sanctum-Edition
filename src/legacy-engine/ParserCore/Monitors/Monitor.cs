...
namespace WaywardGamers.KParser.Monitoring
{
    ...
    public class Monitor
    {
        ...
        /// <summary>
        /// Prevents the RAM reader from opening its legacy process-selection
        /// dialog when it is hosted by the modern, windowless application.
        /// Automatic process discovery and validation remain enabled.
        /// </summary>
        public bool HeadlessMode
        {
            get { return RamReader.Instance.HeadlessMode; }
            set { RamReader.Instance.HeadlessMode = value; }
        }

        /// <summary>
        /// Returns accessible FFXI client process IDs for an explicit memory-offset
        /// detection request. Detection is intentionally unavailable while parsing.
        /// </summary>
        public int[] FindFFXIClientProcessIds()
        {
            if (IsRunning)
            {
                throw new InvalidOperationException(
            ...
            return ProcessAccess.FindFFXIProcessIds();
        }

        /// <summary>
        /// Detects and validates the chat-log memory offset for one FFXI client.
        /// This does not change settings; the caller applies the returned value.
        /// </summary>
        public uint DetectMemoryOffset(int processId)
        {
            if (IsRunning)
            {
                throw new InvalidOperationException(
            ...
            return RamReader.Instance.DetectMemoryOffset(processId);
        }

        /// <summary>
        /// Returns the FFXI process currently supplying RAM chat data, when one
        /// is active. Stat capture uses this to avoid choosing another client.
        /// </summary>
        public int CurrentFFXIProcessId
        {
