    ...
    internal class RamReader : AbstractReader
    {
        ...
        internal int CurrentProcessId
        {
            get
            {
                ...
                try
                {
                    return current == null || current.Process == null || current.Process.HasExited
                        ? 0
                        : current.Process.Id;
                }
                catch (Exception)
        ...
        public override void Start()
        {
            ...
            {
                // A sleeping/background thread is still alive. Starting another reader
                // against the same chat buffer makes both cursors publish every line,
                // which doubles all parsed interactions. Stop always joins the previous
                // worker, and this guard prevents a second worker if it could not exit.
                if (readerThread != null && readerThread.IsAlive)
                {
                    throw new InvalidOperationException(
                        "The memory reader is already running or is still stopping.");
                }
                ...
                try
                {
                    ...
                    if (appSettings.SpecifyPID == true && headlessMode == false)
                    {
        ...
        public override void Stop()
        {
            ...
            lock (readerLifecycleSync)
            {
                threadToStop = readerThread;
                if (IsRunning == false &&
                    (threadToStop == null || threadToStop.IsAlive == false))
                {
                    return;
                }
            ...
            // reset the shared abort event until this worker has actually exited.
            if (threadToStop != null &&
                threadToStop != Thread.CurrentThread &&
                threadToStop.IsAlive)
            {
        ...
        /// <summary>
        /// Performs one non-looping, production-safe scan of a selected client and
        /// returns the first offset whose live chat structure validates. Settings
        /// are deliberately left to the caller so a failed scan cannot alter them.
        /// </summary>
        internal uint DetectMemoryOffset(int processId)
        {
            if (IsRunning)
            {
                throw new InvalidOperationException(
                    "Stop the active parse before detecting a memory offset.");
            }

            lock (memoryOffsetDetectionSync)
            {
                if (IsRunning)
                {
                    throw new InvalidOperationException(
                ...
                POL target = ProcessAccess.OpenFFXIProcess(processId);

                try
                {
                    IList<uint> candidates = ChatLogSignatureScanner.FindCandidateOffsets(target);
                    if (candidates.Count == 0)
                    {
                        throw new InvalidOperationException(
                            "The known chat signature was not found in this FFXI client. " +
                            "The client build may require an updated signature.");
                    }

                    Exception lastValidationError = null;

                    foreach (uint candidateOffset in candidates)
                    {
                        try
                        {
                            ChatLogLocationInfo location = LocateChatLog(target, candidateOffset);
                            ValidateChatLogLocation(target, location);
                            return candidateOffset;
                        }
                        catch (Exception e)
                        {
                            lastValidationError = e;
                            Logger.Instance.Log("Memory offset detection", e.Message);
                        }
                    }

                    throw new InvalidOperationException(
                        "The chat signature was found, but its live chat structure could not be validated. " +
                        "Log fully into a character and try again.", lastValidationError);
                }
                finally
                {
            ...
            try
            {
                ...
                while (!abortMonitorThread.WaitOne(0))
                {
                    ...
                    try
                    {
                        if (needToLogin)
                        {
                            chatLogLocation = LocateChatLogWithFallback();
                            needToLogin = false;
                        }
                    ...
                    catch (Exception e)
                    {
                        ...
                        if (needToLogin)
                        {
                            ...
                            // Signature scanning covers the full module. Avoid repeating
                            // that work twice per second when the client is not ready.
                            abortMonitorThread.WaitOne(4500);
                        }
                    }
                    finally
                    {
                        ...
                        abortMonitorThread.WaitOne(500);
                    }
                }
            }
            catch (Exception e)
        ...
        {
            return ReadChatLogDetails(pol, chatLogInfoAddress);
        }

        private ChatLogDetails ReadChatLogDetails(POL target, IntPtr chatLogInfoAddress)
        {
            IntPtr lineOffsetsBuffer = IntPtr.Zero;

            if ((target == null) || (target.Process == null))
                throw new InvalidOperationException("No FFXI client is available for memory reading.");

            using (ProcessMemoryReading pmr = new ProcessMemoryReading(target.Process.Handle, chatLogInfoAddress, sizeOfChatLogInfoStruct))
            {
        ...
        /// <summary>
        /// Locates the chat log for an explicit client and candidate offset without
        /// mutating the active reader's process or offset fields.
        /// </summary>
        private ChatLogLocationInfo LocateChatLog(POL target, uint candidateOffset)
        {
            if ((target == null) || (target.Process == null))
                throw new InvalidOperationException("No FFXI client is available for memory reading.");

            IntPtr rootAddress = Pointers.IncrementPointer(
                target.FFXIBaseAddress, candidateOffset);
            IntPtr dataStructurePointer = Pointers.FollowPointer(
                target.Process.Handle, rootAddress);

            if (dataStructurePointer == IntPtr.Zero)
            {
                throw new InvalidOperationException(
                    string.Format(
                        "Error dereferencing detected memory offset 0x{0:X8}.",
                        candidateOffset));
            }

            ChatLogControlStruct detectedControl =
                ReadControlStructure(target, dataStructurePointer);

            if (detectedControl.ChatLogInfoPtr == IntPtr.Zero)
            {
                throw new InvalidOperationException(
                    "The detected control structure contains a zero chat info pointer.");
            }

            return new ChatLogLocationInfo(
                dataStructurePointer, detectedControl.ChatLogInfoPtr);
        }

        /// <summary>
        /// Uses the configured offset first, then derives and validates an updated
        /// offset from the live FFXiMain.dll image. The legacy KParser signature is
        /// retained as a final compatibility fallback.
        /// </summary>
        private ChatLogLocationInfo LocateChatLogWithFallback()
        {
            Exception configuredOffsetError = null;

            try
            {
                ChatLogLocationInfo configuredLocation = LocateChatLog();
                ValidateChatLogLocation(configuredLocation);
                return configuredLocation;
            }
            catch (Exception e)
            {
                configuredOffsetError = e;
                Logger.Instance.Log("Configured memory offset", e.Message);
            }

            foreach (uint candidateOffset in ChatLogSignatureScanner.FindCandidateOffsets(pol))
            {
                try
                {
                    IntPtr candidateRoot = Pointers.IncrementPointer(pol.FFXIBaseAddress, candidateOffset);
                    ChatLogLocationInfo candidateLocation = LocateChatLog(candidateRoot);
                    ValidateChatLogLocation(candidateLocation);

                    initialMemoryOffset = candidateOffset;
                    appSettings.MemoryOffset = candidateOffset;
                    appSettings.Save();
                    ...
                    OnReaderStatusChanged(new ReaderStatusEventArgs()
                    {
                        ...
                        StatusMessage = string.Format(
                            "Detected and saved chat memory offset 0x{0:X8}.", candidateOffset)
                    });
                ...
                {
                    Logger.Instance.Log("Detected memory offset", e.Message);
                }
            }

            try
            {
                ChatLogLocationInfo legacyLocation = LocateChatLogViaSignature();
                if (legacyLocation != null)
                {
                    ValidateChatLogLocation(legacyLocation);
                    return legacyLocation;
                }
            }
            catch (Exception e)
            {
                Logger.Instance.Log("Legacy memory signature", e.Message);
            }
        ...
        private void ValidateChatLogLocation(ChatLogLocationInfo location)
        {
            ValidateChatLogLocation(pol, location);
        }

        private void ValidateChatLogLocation(POL target, ChatLogLocationInfo location)
        {
            if (location == null)
                throw new InvalidOperationException("The chat memory location was not found.");

            ChatLogDetails details = ReadChatLogDetails(target, location.ChatLogInfoAddress);
            if (details == null)
                throw new InvalidOperationException("The chat memory structure could not be read.");

            ChatLogInfoStruct info = details.ChatLogInfo;
            if (info.NumberOfLines > 50)
                throw new InvalidOperationException("The configured offset does not contain a valid chat line count.");

            if ((info.ChatLogBytes < 0) || (info.ChatLogBytes > 65536))
                throw new InvalidOperationException("The configured offset does not contain a valid chat buffer size.");

            if ((info.FinalOffset < 0) || (info.FinalOffset > info.ChatLogBytes))
                throw new InvalidOperationException("The configured offset does not contain a valid chat buffer offset.");

            if ((info.ChatLogBytes > 0) && (info.PtrToCurrentChatLog == IntPtr.Zero))
                throw new InvalidOperationException("The configured offset does not contain a chat buffer pointer.");
        }
        ...
        {
            return ReadControlStructure(pol, dataStructurePointer);
        }

        private ChatLogControlStruct ReadControlStructure(POL target, IntPtr dataStructurePointer)
        {
            ...
            using (ProcessMemoryReading pmr = new ProcessMemoryReading(target.Process.Handle, dataStructurePointer, sizeOfChatLogControlStruct))
            {
                if (pmr.ReadBufferPtr == IntPtr.Zero)
