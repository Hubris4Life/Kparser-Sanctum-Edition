...
namespace WaywardGamers.KParser.Parsing
{
    internal class MsgManager
    {
        ...
        private MsgManager()
		{
            // Keep the authoritative database close to the live reader. The
            // message-age guard below still protects multi-line action joins;
            // this timer only controls how soon an already-safe message is
            // made visible to reports.
            periodicUpdates = new System.Timers.Timer(1000.0);
        ...
        {
            // Spell/ability matching searches up to ten seconds behind the
            // newest line. Retain one additional second so a boundary event
            // cannot be committed while a valid continuation can still join.
            DateTime shortCheckTime = DateTime.Now.ToUniversalTime() - TimeSpan.FromSeconds(11);
