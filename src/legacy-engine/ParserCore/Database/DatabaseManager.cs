...
namespace WaywardGamers.KParser
{
    public sealed class DatabaseManager : IDisposable
    {
        ...
        {
            get { return Interlocked.Read(ref snapshotRevision); }
        }
        ...
        {
            KPDatabaseDataSet database;
            return TryGetDatabaseForReading(out database) ? database : null;
        }

        /// <summary>
        /// Attempts to acquire read access while also reporting whether the mutex
        /// was acquired. This lets background readers release the mutex safely
        /// even when no database is currently open.
        /// </summary>
        internal bool TryGetDatabaseForReading(out KPDatabaseDataSet database)
        {
            database = null;

            // Wait for up to 2 seconds to try to acquire the mutex.
            if (databaseAccessMutex.WaitOne(2000) == false)
                return false;
